...
def scan_vulnerability(request):
    try:
        ...
        # Add warning if gobuster had issues
        if 'warning' in gobuster_results:
            structured_data['gobuster_warning'] = gobuster_results['warning']
...
def run_gobuster(url):
    ...
    try:
        cmd = [
            ...
            "-r",  # Follow redirects
            "-b", "200,204,301,302,307,403,500",  # Exclude common false positive status codes
            "--exclude-length", "3096",  # Exclude the specific length mentioned in error
            "--random-agent",  # Use random user agent
            "--timeout", "10s",  # Set timeout per request
        ...
        logger.error(f"Gobuster scan failed: {error_msg}")
        
        # If status code exclusion failed, try a simpler approach
        if "status code that matches" in error_msg:
            logger.info("Retrying Gobuster with simpler configuration")
            try:
                # Simpler command without status code exclusions
                cmd_simple = [
                    "gobuster", "dir",
                    "--url", url,
                    "--wordlist", wordlist,
                ...
                process = subprocess.run(
                    ...
                    stderr=subprocess.PIPE,
                    text=True,
                    timeout=300,  # 5-minute timeout for retry
                    check=True
                )
                ...
                # Parse and filter results manually
                parsed_results = parse_gobuster_results(results)
                
                # Filter out results with status 200 if they seem suspicious
                filtered_dirs = []
                filtered_files = []
                
                for directory in parsed_results['directories']:
                    if directory['status'] != 200 or len(filtered_dirs) < 10:  # Keep some 200s but not too many
                        filtered_dirs.append(directory)
                
                for file in parsed_results['files']:
                    if file['status'] != 200 or len(filtered_files) < 10:  # Keep some 200s but not too many
                        filtered_files.append(file)
                
                return {
                    "directories": filtered_dirs,
                    "files": filtered_files,
                    "warning": "Target site returns 200 status for non-existent URLs. Results may contain false positives and have been filtered."
                }
                
            except Exception as retry_error:
                logger.error(f"Gobuster retry also failed: {str(retry_error)}")
